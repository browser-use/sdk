...
description: Copy one blob into your AI editor. Your agent knows the entire API.
icon: brain
---
...
- `workspace_id: str` — attach a workspace for persistent file storage
...
for f in files.files:
    print(f.path, f.size, f.url)
await client.workspaces.delete_file(str(workspace.id), path="output.csv")

# Workspace management
workspaces_list = await client.workspaces.list()
# WorkspaceListResponse has: .items, .total_items, .page_number, .page_size
ws = await client.workspaces.get(str(workspace.id))
await client.workspaces.update(str(workspace.id), name="renamed")
await client.workspaces.delete(str(workspace.id))

# Session messages — retrieve full conversation history
msgs = await client.sessions.messages(str(session.id), limit=50, after=None, before=None)
# MessageListResponse has: .messages (list of MessageResponse)
# Each MessageResponse has: .id, .role ("user"/"assistant"/"tool"), .data (JSON string)
...
await client.run("Save results to output.csv", { workspaceId: workspace.id });

const wsFiles = await client.workspaces.files(workspace.id, { includeUrls: true });
await client.workspaces.deleteFile(workspace.id, "output.csv");
await client.workspaces.update(workspace.id, { name: "renamed" });
await client.workspaces.delete(workspace.id);

// Session messages
const msgs = await client.sessions.messages(session.id, { limit: 50 });
for (const m of msgs.messages) {
  console.log(`[${m.role}] ${m.data}`);
}
```
...
- `workspaceId` — attach a workspace for persistent file storage
...
- **Workspaces**: persistent file storage across sessions. Create a workspace, attach to tasks via `workspace_id`, list/delete files. CRUD: `client.workspaces.create/get/update/list/delete`
- **Session messages**: `client.sessions.messages(session_id)` returns the full conversation history (user, assistant, tool messages). Supports cursor-based pagination with `after`/`before`/`limit`
