...
Paste the following into **Cursor**, **Claude Code**, **Windsurf**, or any coding agent. It contains the full SDK reference — every method, every parameter, working examples.
...
const result = await client.run("Get product info", { schema: Product });
...
- **ChatBrowserUse**: the chat completion API endpoint, always routes to the best model
