...
Every session has a `liveUrl` for real-time monitoring and a shareable public URL.
...
See the [API Reference](/api-reference) for the full list of session parameters (`profile_id`, `proxy_country_code`, `browser_screen_width`, etc.).
