...
## Built for EU workloads

Exciting news: we built an EU deployment for teams that need their browser and
agent workloads to run in Europe.

You can run V4 agents and cloud browsers in Frankfurt, with prompts, screenshots,
browser sessions, profiles and stored run artifacts kept with the EU workload
when they are retained. Keep using the same Browser Use account and project.
Billing, account administration and shared account data remain in the US, so
you do not need to create a second account or move your existing billing setup.

The EU deployment has a separate dashboard, API endpoint and API key. The guide
above explains the exact data boundaries, provider paths and remaining
exceptions so your security team can make an informed decision.

If you need another feature, have a question about data policies, or want to
validate a new use case, let us know. We would love to help unblock you and to
unblock new use cases.
