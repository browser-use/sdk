...
Paste the prompt into **Claude Code**, **Codex**, **Cursor**, or any coding agent — your agent will be able to write Browser Use Cloud code immediately.

<Tip>
  In Claude Code: save as `CLOUD.md` in your project root. In Cursor: paste into a `.cursorrules` file or chat context.
</Tip>

<Accordion title="Copy full SDK prompt" icon="clipboard">
````markdown
# Browser Use Cloud SDK

Both APIs are in the same package (`browser-use-sdk`):
- **BU Agent API (v3)** — `browser-use-sdk/v3`
