...
- **Minimum top-up:** `$1.00` USDC per request
...
Each x402 payment adds `$1` of credits to your project. When credits hit zero, the next request returns `402`, and the SDK automatically signs another `$1` payment to keep going. **You don't manage top-ups manually; just make sure your wallet has enough USDC for your expected usage.**
...
result = await client.run("...")          # $1 USDC charged, credited to the API key's project
```
```typescript TypeScript
import { BrowserUse } from "browser-use-sdk/v3";
...
Your code asks for something, we say "$1 please", your wallet pays automatically, we run your request.

A bit more detail:

1. Your code makes a request (e.g. "run this task").
2. If your project has no credits, we respond with "Payment required: $1".
3. The SDK auto-signs a `$1` USDC payment from your wallet and resends the request.
4. Coinbase moves the `$1` on-chain. We add `$1` to your project's credit balance.
