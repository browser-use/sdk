...
|---|---:|
| Creates, lists, updates, deletes, event reads, and full run reads | 100 requests/second |
...
The project limiter counts requests in **five-second windows**. A 100 RPS
budget therefore permits 500 requests per window. `X-RateLimit-Limit` reports
that window allowance, not requests per second: a value of `500` does not mean
500 RPS. `X-RateLimit-Remaining` is the window's remaining request count.
...
three-second interval average about 33 event requests per second, a third of
the standard 100 RPS general budget; three hundred such waits would exceed it. At that scale use a
