...
The standard per-project application buckets both scale with your stored concurrency grant:

| Requests | Default request budget |
|---|---:|
| Creates, lists, updates, deletes, event reads, and full run reads | `max(25, 2 × stored concurrency)` requests/second |
...
`GET /api/v4/runs/{run_id}/events` and `GET /api/v4/runs/{run_id}` use the general bucket, which shares the poll bucket's formula but is counted separately. Account overrides and other protections can differ from these defaults; use the response headers and error details when diagnosing throttling.

For example, a project with 500 stored concurrent sessions and no request-rate
override has a 1,000 RPS status-polling budget and a separate 1,000 RPS general
