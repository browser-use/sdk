...
The session message queue accepts up to **20 pending messages**. A full queue returns **429**. It is a queue for that conversation, not a project-wide batch queue or a reservation of browser capacity.
...
The standard per-project application buckets both scale with your stored concurrency grant. The general bucket is capped at 100 RPS; higher general budgets are account overrides:

| Requests | Default request budget |
|---|---:|
| Creates, lists, updates, deletes, event reads, and full run reads | `max(25, 2 × stored concurrency)` requests/second, capped at 100 |
...
`GET /api/v4/runs/{run_id}/events` and `GET /api/v4/runs/{run_id}` use the general bucket, which shares the poll bucket's formula up to its 100 RPS cap and is counted separately. Account overrides and other protections can differ from these defaults; use the response headers and error details when diagnosing throttling.

For example, a project with 500 stored concurrent sessions and no request-rate
override has a 1,000 RPS status-polling budget and a separate 100 RPS general
budget. A free or Tier 0 project (10 concurrent sessions) sits on the 25 RPS floor
of both. All API keys in that project share those budgets. Creating a new key
...
the 25 RPS general floor of a Tier 0 project. For that workload, explicitly use a
...
| 429 with a queue-full message in `detail` | This conversation has 20 pending messages | Wait for its queue to drain or cancel an appropriate pending message. |
