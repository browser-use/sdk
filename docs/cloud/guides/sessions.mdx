...
description: Stateful browsers, persistent login state, profile sync.
icon: browser
---
...
See [Browser Infrastructure](/cloud/guides/browser-api) for the full list of session parameters (`profile_id`, `proxy_country_code`, `browser_screen_width`, etc.).
...
<Warning>
  Profile state (cookies, localStorage) is only saved when the session ends. Always call `sessions.stop()` when you're done — if a session is left open or times out, changes may not be persisted.
</Warning>
