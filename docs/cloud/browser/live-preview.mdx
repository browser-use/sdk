...
Enable recording to get an MP4 video of the session after it completes. Recording URLs may take a few seconds to appear after the session stops — poll `sessions.get()` if empty initially.
...
print(result.recording_url)  # presigned MP4 download URL
```
```typescript TypeScript
import { BrowserUse } from "browser-use-sdk/v3";
...
console.log(result.recordingUrl); // presigned MP4 download URL
```
</CodeGroup>
