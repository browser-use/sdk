...
# Recording may not be ready immediately — poll if empty
import asyncio
while not result.recording_urls:
    await asyncio.sleep(2)
    result = await client.sessions.get(str(result.id))
...
// Recording may not be ready immediately — poll if empty
let session = result;
while (!session.recordingUrls?.length) {
