...
Enable recording to get an MP4 video of the browser session. Only available when the agent actually opens a browser — tasks answered without browsing produce no recording. Poll `sessions.get()` after the session stops if `recording_urls` is empty — processing may take a few seconds for longer sessions.
