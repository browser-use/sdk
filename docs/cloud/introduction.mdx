...
<Frame>
  <img src="/cloud/images/concepts-overview.png" alt="Browser Use Cloud Overview" style={{ borderRadius: '0.5rem' }} />
</Frame>
...
<Tip>
  Start with **Tasks** — it is the fastest path to value. Move to **Skills** when you need a repeatable, production-grade endpoint.
</Tip>
...
You can also pass `cloud_proxy_country_code` and `cloud_profile_id` for geo-routing and persistent login state. See the [open-source docs](/open-source/customize/browser/remote) for full options.

## Core concepts

- **Task** — a natural language prompt the AI agent executes. The primary resource you will use.
...
<CardGroup cols={2}>
  <Card title="Tasks" icon="play" href="/cloud/guides/tasks">
    Structured output, streaming, and parameters.
  </Card>
  <Card title="Skills" icon="wand-magic-sparkles" href="/cloud/guides/skills">
