...
[OpenClaw](https://openclaw.ai) is a self-hosted gateway that connects chat apps like WhatsApp, Telegram, and Discord to AI coding agents. Add Browser Use and those agents can control Browser Infrastructure — hosted cloud browsers for AI agents with managed profiles, proxy routing, supported CAPTCHA handling, and live viewing.
...
Replace `<BROWSER_USE_API_KEY>` with your actual key. These Browser Use session
parameters can be passed as query parameters in the `cdpUrl`:

- `timeout` — requested session duration in minutes (1–240; defaults to 60)
...
See the [browser reference](/cloud/browser/quickstart) for maintained settings
and [pricing](https://browser-use.com/pricing) for current commercial terms.
