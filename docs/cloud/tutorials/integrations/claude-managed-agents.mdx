...
description: Give Anthropic's Claude Managed Agents Browser Infrastructure through the Browser Use CLI.
---

[Claude Managed Agents](https://platform.claude.com/docs/en/managed-agents) run on Anthropic's hosted platform. Install the `browser-use` CLI in the agent's environment and it can drive Browser Infrastructure — hosted cloud browsers for AI agents with managed proxy routing, supported CAPTCHA handling, live view, and recording. Your API key stays in a credential vault; the model never sees it.
...
model:
  id: claude-opus-4-8
description: Drives Browser Infrastructure with the Browser Use CLI.
system: |
