...
<CodeGroup>
```python Python
# Create a separate workspace for each platform
rula_ws = await client.workspaces.create(name="rula-search")
grow_ws = await client.workspaces.create(name="grow-therapy-search")

async def search_platform(task: str, ws_id: str, schema):
    return await client.run(task, workspace_id=ws_id, schema=schema)

# Run both searches concurrently
rula_result, grow_result = await asyncio.gather(
    search_platform(
...
```typescript TypeScript
// Create a separate workspace for each platform
const rulaWs = await client.workspaces.create({ name: "rula-search" });
const growWs = await client.workspaces.create({ name: "grow-therapy-search" });
...
const [rulaResult, growResult] = await Promise.all([
  client.run(
    "Go to rula.com and search for therapists who accept {{Aetna}} insurance " +
    "and specialize in {{anxiety}}. Return the first 5 results as JSON.",
    { workspaceId: rulaWs.id, schema: TherapistResults },
  ),
  client.run(
    "Go to growtherapy.com and search for therapists in {{New York}} " +
    "who specialize in {{anxiety}} and accept insurance. " +
    "Return the first 5 provider profiles as JSON.",
    { workspaceId: growWs.id, schema: ProviderSearch },
  ),
]);
