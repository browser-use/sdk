...
After the first run caches the search flow, sweep across different parameters
without another LLM generation step:
...
| Step | What happens |
|------|-------------|
| First search | Agent navigates Grow Therapy and caches the flow |
| 12 cached sweeps (4 cities x 3 specialties) | Script reruns with new parameters without another LLM generation step |
| Site layout change | The [saved script](/cloud/agent/scripts) can be repaired and retested |

<Info>
Therapy platforms have dynamic UIs that can change frequently. A later agent run can repair and retest the [saved script](/cloud/agent/scripts) when the site changes.
</Info>
