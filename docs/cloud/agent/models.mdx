...
To default every session on a client to BYOK, pass it once to the constructor: `AsyncBrowserUse(use_own_key=True)` (Python) or `new BrowserUse({ useOwnKey: true })` (TypeScript).
