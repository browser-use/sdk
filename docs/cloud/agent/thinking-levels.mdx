...
See [Models](/cloud/agent/models) for the complete V4 model list, default
...
| `gpt-6-sol`, `gpt-6-luna` | `reasoning.effort` | `none`, `low`, `medium`, `high`, `xhigh`, `max` |
...
| `claude-opus-5-5` | `output_config.effort` | `low`, `medium`, `high`, `xhigh`, `max` |
| `claude-opus-5-5` | `thinking.type` | `adaptive` |
| `claude-opus-5-5` | `thinking.display` | `omitted`, `summarized` |
| `deepseek-v4.1-flash` | `thinking.type` | `enabled`, `disabled` |
| `deepseek-v4.1-flash` | `reasoning_effort` | `low`, `high`, `max` |
| `mimo-v2.6-pro`, `mimo-v2.6-flash` | `thinking.type` | `enabled`, `disabled` |
...
<Note>
  ...
  example, `gpt-5.6-luna`, `gpt-6-astra`, and `gpt-6-sol` default to
  `reasoning.effort: xhigh`; `gpt-6-luna` defaults to `medium`.
  `deepseek-v4.1-flash` and the MiMo models default to `thinking.type: disabled`.
