...
See [Models](/cloud/agent/models) for the V4 model list and BYOK routes. See
[pricing](https://browser-use.com/pricing) for current token rates.
...
| `bu-2-0-mini-preview`, `gpt-4.1`, `gpt-4.1-mini`, `glm-5.2`, `minimax-m3`, `llama-4-maverick-17b-128e-instruct`, `claude-3-7-sonnet-20250219` | — | — | — | — |
...
setting when the field is omitted; send `null` to clear it. Generated V2 and V3
SDK clients expose the field as `thinking_level` in Python and `thinkingLevel`
in TypeScript. Generated V4 SDK clients expose `model_params` in Python and
`modelParams` in TypeScript.
