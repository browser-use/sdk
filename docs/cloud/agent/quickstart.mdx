...
`client.run()` creates a session, polls every 2 seconds until completion (up to 300 seconds), and returns the result. It accepts all parameters from the [Create Session](/cloud/api-v3/sessions/create-session) endpoint.

The result has: `output` (string or parsed schema), `status`, `id`, `total_cost_usd`, `live_url`, `recording_urls`, and all other [session fields](/cloud/api-v3/sessions/get-session).
