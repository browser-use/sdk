...
Script caching lets you run a browser task once with a full agent, then **re-execute the same task instantly** using a cached deterministic script — no LLM, up to 99% cheaper.
...
<Steps>
  <Step title="You send a task with {{brackets}}">
    The brackets mark which parts are parameters. The system strips the values to create a **template**.
