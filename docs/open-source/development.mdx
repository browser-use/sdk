...
Please note that each CLI release is associated with a specific version of Mintlify. If your local website does not align with the production version, please update the CLI:
...
<Frame>
  <img src="/open-source/images/checks-passed.png" style={{ borderRadius: '0.5rem' }} />
</Frame>

## Code Formatting

We suggest using extensions on your IDE to recognize and format MDX. If you are a VSCode user, consider the [MDX VSCode extension](https://marketplace.visualstudio.com/items?itemName=unifiedjs.vscode-mdx) for syntax highlighting, and [Prettier](https://marketplace.visualstudio.com/items?itemName=esbenp.prettier-vscode) for code formatting.
...
2. **Release a new version**:
   - If the tag is a pre-release (`x.x.xrcXX`), the package is published to PyPI as a pre-release.
   - If the tag is a stable release (`x.x.x`), the package is published to PyPI as a stable release, and the `stable` branch is updated to match the release.
