...
### Vision & Processing
- `use_vision` (default: `True`): Vision mode - `"auto"` includes screenshot tool but only uses vision when requested, `True` always includes screenshots, `False` never includes screenshots and excludes screenshot tool
...
- `max_actions_per_step` (default: `5`): Maximum actions per step, e.g. for form filling the agent can output 4 fields at once. We execute the actions until the page changes.
- `max_failures` (default: `5`): Maximum retries for steps with errors
...
- `llm_timeout` (default: `None`): Timeout in seconds for LLM calls. When omitted, the agent selects a model-dependent timeout of 30, 75, or 90 seconds
...
| Variable | Default | Description |
|----------|---------|-------------|
| `TIMEOUT_NavigateToUrlEvent` | `30.0` | Timeout for page navigation |
