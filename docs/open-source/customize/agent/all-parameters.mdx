...
### Core Settings
- `tools`: Registry of [tools](/open-source/customize/tools/available) the agent can call. [Example](/open-source/customize/tools/basics)
- `skills` (or `skill_ids`): List of skill IDs to load (e.g., `['skill-uuid']` or `['*']` for all). Requires `BROWSER_USE_API_KEY`. [Docs](/open-source/customize/skills/basics)
